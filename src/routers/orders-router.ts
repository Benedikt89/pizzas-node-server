...
router.get(`/`, async (ctx: any, next: any) => {
    const interviews = await ordersRepository.getAllInterviews();
    ctx.body = interviews;
});
...
router.post(`/`, async (ctx: any, next: any) => {
    ...
    let dev = await pizzasRepository.getDeveloper(interview.developerId);
    if (dev == null) {
        ctx.status = 406;
        return;
    }
    ...
    const developers = await ordersRepository.createInterview(interview);
    ctx.body = developers;
});
